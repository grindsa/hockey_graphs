...
def teamcomparison_get(logger, request, fkey=None, fvalue=None):
    ...
    result.append(_pdo_breakdown_get(logger,  _('PDO breakdown'), request, teamstat_dic, teams_dic))
    # result.append(banana_chart1_create(logger, 'foo1'))
    result.append(banana_chart2_create(logger, 'foo2'))

    return result

def _pdo_breakdown_get(logger, title, request, teamstat_dic, teams_dic):
    """ pdo breakdown """
    logger.debug('_pdo_breakdown_get()')

    # get data for chart
    breakdown_dic = pdo_breakdown_data_get(logger, teamstat_dic, teams_dic)
