...
def process_match(match_dic):
    """ process match dictionary """
    # add teams if not existing
    if match_dic['home_id'] not in team_list:
        team_id = team_add('team_id', match_dic['home_id'], {'team_id': match_dic['home_id'], 'team_name': match_dic['home_name'], 'shortcut': match_dic['home_shortcut']})
        team_list.append(team_id)
    if match_dic['visitor_id'] not in team_list:
        team_id = team_add('team_id', match_dic['visitor_id'], {'team_id': match_dic['visitor_id'], 'team_name': match_dic['visitor_name'], 'shortcut': match_dic['visitor_shortcut']})
        team_list.append(team_id)
    ...
    if int(match_dic['id']) not in match_list:
        ...
        match_id = match_add('match_id', match_dic['id'], {'match_id': match_dic['id'], 'date': match_dic['date'], 'home_team_id': match_dic['home_id'], 'visitor_team_id':  match_dic['visitor_id'], 'season_id': season_id})
    ...
    for shot in match_dic['shots']:
        ...
        if shot['player_id'] not in player_list:
            ...
            player_id = player_add('player_id', shot['player_id'], {'player_id': shot['player_id'], 'first_name': shot['first_name'], 'last_name': shot['last_name'], 'jersey': shot['jersey']})
            player_list.append(player_id)

        # add shot
        zone = zone_name_get(shot['coordinate_x'], shot['coordinate_y'])
        ...
        shot_id = shot_add('shot_id', shot['id'], data_dic)
...
if __name__ == '__main__':
    ...
    # get list of matches
    match_list = match_list_get(None, None, ['match_id'])
